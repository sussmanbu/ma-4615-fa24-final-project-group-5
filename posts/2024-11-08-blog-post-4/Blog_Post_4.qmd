...
long_data <- cleaned_data %>%
  pivot_longer(
    cols = matches("^(WHT|BLKT|HISPT|ASIANT|AIANT|NHOPIT|TOMRT|MT|FT)([1-9]|1_2)?$"),
    names_to = c("Race", "JobType"),
    names_pattern = "^(WHT|BLKT|HISPT|ASIANT|AIANT|NHOPIT|TOMRT|MT|FT)([1-9]|1_2)?$",
    values_to = "Count"
  ) %>%
...
long_data <- cleaned_data %>%
  pivot_longer(
    cols = matches("^(Male|Female|Other|MT|FT)[1-9](_2)?$"),
    names_to = c("Gender", "JobType"),
    names_pattern = "^(Male|Female|Other|MT|FT)([1-9]|1_2)?$",
    values_to = "Count"
  ) %>%
...
# Plotting
ggplot(filtered_data, aes(x = Gender, y = Count, fill = Gender)) +
  geom_bar(stat = "identity") +
  facet_wrap(~ JobType, scales = "free_y") +
  theme_gray() +
  labs(
